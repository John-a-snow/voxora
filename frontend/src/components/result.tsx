    ...
    return (
        <div className="flex flex-col gap-12 mt-12">
            ...
            <section className="border-4 border-brand-border bg-white p-8 md:p-12 shadow-[8px_8px_0px_0px_rgba(23,23,23,1)]">
                ...
                <p className={`text-2xl md:text-4xl font-mrdium leading-relaxed ${isRefused ? 'text-gray-500 italic' : 'text-gray-900'}`}>
                    {isRefused
            ...
            {result.retrieved_documents && result.retrieved_documents.length > 0 && (
                <section className="mt-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {result.retrieved_documents.slice(0, 5).map(document, idx) => (
                            <div key={idx} className="border-2 border-brand-border p-6 bg-white flex flex-col h-full">
                                <div className="flex justify-between items-start mb-4">
                                    <span className="text-xl font-black text-brand-primary">{(idx +1).toString().padStart(2, '0')}</span>
                                    <div className="text-right">
                                        <div className="text-xs font-mono bg-gray-100 px-2 py-1">{doc.document_id}</div>
                                        <div className="text-[10px] uppercase font-bold text-gray-500 mt-1">Scare: {doc.score?.toFixed(4)}
                                        </div>
                                    </div>
                                    ...
                                    <div className="text-[10px] uppercase font-bold tracking-widest border-t border-gray-200 pt-3 text-gray-400">
                                        {doc.source || 'Knowledge Base'} . {document.language || 'Unknown'}
                                    </div>
                                </div>
                            </div>
                        )}
